...
오직미(OZICME) 쌀을 사용하는 식당을 발견하고 바로 예약할 수 있는 정적 사이트입니다. 로그인 없이 누구나 이용할 수 있으며, 개인정보와 거래정보는 공개하지 않습니다.
...
- 홈: `http://localhost:8000/index.html`
- 식당 리스트: `http://localhost:8000/restaurants.html`

## JSON 데이터 추가/수정 방법

`public-restaurants.json`에 식당 정보를 추가합니다. 필수 필드는 기존 데이터(`name`, `region`, `riceVarieties`, `firstOrder`, `lastOrder`, `purchaseCount`)이며, 소비자용 확장 필드는 누락되어도 UI가 깨지지 않도록 `미등록`으로 표시됩니다.
...
```json
{
  ...
  "images": {
    ...
    "gallery": ["https://images.unsplash.com/photo-1525755662778-989d0524087e?auto=format&fit=crop&w=800&q=80"]
  },
  "verifiedAt": "2024-08"
}
```
...
- `images.thumbnail`: 카드 및 상세 대표 이미지로 사용됩니다.
- `verifiedAt`: 최근 확인 월(YYYY-MM)로 입력합니다.
